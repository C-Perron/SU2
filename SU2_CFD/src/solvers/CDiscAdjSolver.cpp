...
void CDiscAdjSolver::ExtractAdjoint_Variables(CGeometry *geometry, CConfig *config) {
  ...
  if ((config->GetKind_Regime() == ENUM_REGIME::COMPRESSIBLE) && (config->GetnMarker_EngineInflow() > 0)) {
    ...
    SU2_MPI::Allreduce(
      Local_Sens_EngineInflow_Target.data(),
      Total_Sens_EngineInflow_Target.data(),
      Total_Sens_EngineInflow_Target.size(),
  ...
  if ((config->GetKind_Regime() == ENUM_REGIME::COMPRESSIBLE) && (config->GetnMarker_EngineExhaust() > 0)) {
    ...
    SU2_MPI::Allreduce(
      Local_Sens_Exhaust_Temperature_Target.data(),
      Total_Sens_Exhaust_Temperature_Target.data(),
      Total_Sens_Exhaust_Temperature_Target.size(),
      MPI_DOUBLE, MPI_SUM, SU2_MPI::GetComm()
    );
    SU2_MPI::Allreduce(
      Local_Sens_Exhaust_Pressure_Target.data(),
      Total_Sens_Exhaust_Pressure_Target.data(),
      Total_Sens_Exhaust_Pressure_Target.size(),
      MPI_DOUBLE, MPI_SUM, SU2_MPI::GetComm()
    );
  }
